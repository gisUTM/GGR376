...
```{r}
house_prices <- read_csv("https://raw.githubusercontent.com/gisUTM/GGR376/master/house_prices.csv")
house_attributes <- read_csv("https://raw.githubusercontent.com/gisUTM/GGR376/master/house_attributes.csv", guess_max = 9500) 
```
...
```{r}
variable_attributes <- read_csv("https://raw.githubusercontent.com/gisUTM/GGR376/master/Attribute_Definitions.txt")
variable_attributes
```
...
We will demonstrate the first data transformation for houseValue
...
We will begin with a manual approach to variable selection.
- Begin with the strongest predictor variable based on the correlation plot, which appear to be median age.
...
We see that our predictor is statistically significant and so is the the overall model.
The adjusted R-Squared is 0.36, thus we can explain 36% of the variation in the dependent variable with our model.
...
If you do not want your numbers to be in scientific notation, run the following command
```{r}
options(scipen=999)
```
...
We are looking for a flat line in the first and third plots.
...
It is apparent that observations 402, 405 and 471 are causing some issues in
our model. Take a look at their values by using the slice function.
...
Recall from the boxplot of MedianAge that these are extreme values. Let's remove
them from the set as they do not represent the overall distribution of age in study area and refit the model.

```{r}
sd_trans_sub <- sd_trans%>%
  # We can use a minus sign to include everything in the slice except those two rows.
  slice(-c(402, 471, 405))

model_2 <- lm(logValue ~ MedianAge, sd_trans_sub)
summary(model_2)
```
```{r}
par(mfrow=c(2,2)) # set 2 rows and 2 column plot layout
...
Before we move on, let's try adding in an additional variables.
- nBedrooms

```{r}
model_3 <- lm(logValue ~ MedianAge + nBedrooms, data = sd_trans_sub)

summary(model_3) # We increased our Adj.R.SQ by ~0.04
```
...
summary(model_4) 
# No increase to our Adj.R.SQ
# Also it is not staitically significant
...
-nBedrooms is no longer significant and it is suffering from multicollinearity as its sign flipped.
    + Confirm this with car::vif
...
# Spend ten minutes trying out additional models and using the summary and plot.

### We will work with model_3
...
- No issues of correlation

### Normality of residuals

- We could check it with the normal Q-Q plot or we can create a histogram.
- With our large sample size it is possible to relax this assumption
...
We will accept this deviation in our case from normality because of our large sample size. 
    
#### Auto-correlation of residuals

This is geographical data and we need to test for spatial correlation of our residuals. At the moment our data is not geographical, so we will need to introduce this into our data. The steps for checking for spatial auto-correlation include:

1. Import spatial boundaries.
2. Join or data and residuals.
3. Moran's I calculation for residuals.
4. If spatial-autocorrelation occurs, use a spatial-error or spatial-lag model.

### Spatial Boundaries
We have a number of classes for spatial data in R, which are included in the sp package.

- SpatialPoints or SpatialPointsDataFrame
- SpatialLines or SpatialLinesDataFrame
- SpatialPolygons or SpatialPolygonsDataFrame

For each of these, the first class is only able to store geometry. The second class that include Spatial***DataFrame allows you to store variables along with the geometry.
...
The polygon data is stored on the GitHub page and can be opened using the readOGR function in the rgdal package
...
```{r}
california_polygons <- readOGR("https://raw.githubusercontent.com/gisUTM/GGR376/master/house_polygons.geojson")
```
...
Take a look at the Data.Frame. It contains one variable GEOID, which we can use in joining our data and results.
```{r}
...
- Join the new dataframe to the sp object.
```{r}
...
- This is a pretty ugly map, let's explore different colour choices.

```{r fig.height=7}
display.brewer.all()
```

- Create a palette to be used in our map with 7 colours (6 breaks)

### Automated Variabel Selection
```{r}
...
- It appears that there is a strong spatial relationship with the residuals as we move away from the water. 

#### Confirm Spatial Auto-Correlation with Moran's I
- Moran's I requires we calculate spatial weights
    + Points: most often distances
    + Polygons: Neighbours or distances

- We calculate the neighbours list with spdep::poly2nb()
...
- It is pretty mess on the west coast, but this demonstrates the relationships stored in the neighbours list.
...
#### Spatial Regression Models: Spatial Lag and Spatial Error
- In class we have discussed the two types of spatial models.
