...
mtcars <- read_csv("//Users/matthewadams/Desktop/mtcars.csv")
...
1. Split the label variable into three variables:
    ...
    2. County
    3. State
2. Remove any extra spaces before or after a county name, which can occur when 
splitting strings.
...
Observations 402, 405 and 471 are causing some issues in
...
class(california_spdf)
california_spdf@data
```
...
Spatial auto-correlation is suggested by this plot. 
...
```{r}
lanDiag <- lm.LMtests(model_3, california_listw, test="all")
summary(lanDiag)
```
